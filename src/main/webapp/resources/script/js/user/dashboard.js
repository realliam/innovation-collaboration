...
cj(".dashboard-content").ready(function() {
	...
	var sortableDashboard = cj(".dashboard-container").sortable({
		placeholder : 'sortable-placeholder-hightlight',
		containment : 'body',
		opacity : '0.25',
		revert : 500,
		distance : 50,
	...
	var saveDashboardLayoutInBackend = function() {
		var layoutSequence = '';
		var panels = sortableDashboard.children('.dashboard-panel-flag');
		
		panels.each(function(index, element) {
			if(index==0){
				layoutSequence = layoutSequence + cj(this).attr('id');
			} else {
				layoutSequence = layoutSequence + ',' + cj(this).attr('id');
			}
		...
		cj.ajax({
			type : "POST",
			url : "/asyncConfig/saveLayout",
			data : "value=" + layoutSequence,
	...
	cj("#editProfile").click(function(){
		cj("#edit-my-profile").parent().find(".ui-dialog-titlebar").addClass("edit-profile-title");
		cj("#edit-my-profile").dialog("open");
	});
	
	cj("#adminPanel").click(function(){
		cj("#adminEntry").parent().find(".ui-dialog-titlebar").addClass("user-admin-title");
		cj("#adminEntry").dialog("open");
	});
});