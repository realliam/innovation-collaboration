...
cj("#adminEntry").ready(function() {
	...
	function loadParentTeamOptionsForAddTeam(selected){
		cj.ajax({
			type: "POST",
			...
			cj.ajax({
				...
				success:function(response){
					if(response.status == "SUCCESS"){
						...
						loadParentTeamOptionsForAddTeam(parentTeamName);
					}else{
	...
	function userTeamSave(){
		...
		cj.ajax({
			...
			success : function(response) {
				...
				if(response.status == "SUCCESS"){
					msg.text("update user team successful!");
				}else{
					msg.text("update user team fail!");
				}
			}
		});
	}
	...
	function userRoleSave(){
		...
		cj.ajax({
			...
			success : function(response) {
				var msg = cj("#manageUserMsg");
				if(response.status == "SUCCESS"){
					msg.text("update user role successful!");
				}else{
					msg.text("update user role fail!");
				}
			}
		});
	}
	...
	function userStatusSave(){
		...
		cj.ajax({
			...
			success : function(response) {
				var msg = cj("#manageUserMsg");
				if(response.status == "SUCCESS"){
					msg.text("update user status successful!");
				}else{
					msg.text("update user status fail!");
				}
			}
		});
	}
